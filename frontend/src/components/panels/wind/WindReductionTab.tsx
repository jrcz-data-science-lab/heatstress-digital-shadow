...
export function WindReductionTab() {
	...
	const handleGenerateWindMap = () => {
		...
		const treesMinHeightValue = getValidatedValue(
			requirePositiveNumber(treesMinHeight, 'Trees minimum height must be a positive number'),
			request.setError,
		);
		...
		const rasterResolutionValue = getValidatedValue(
			requirePositiveNumber(rasterResolution, 'Raster resolution must be a positive number'),
			request.setError,
		);
		...
		const referenceHeightValue = getValidatedValue(
			requirePositiveNumber(referenceHeight, 'Reference height must be a positive number'),
			request.setError,
		);
		...
		const targetHeightValue = getValidatedValue(
			requirePositiveNumber(targetHeight, 'Target height must be a positive number'),
			request.setError,
		);
		...
		<div className="wind-panel__tab-content">
			...
			>
				...
				>
					...
					<ol>
						<li>
							<strong>Height Map:</strong> Creates a height difference map from DSM
							and DTM
						</li>
						<li>
							<strong>Building & Tree Import:</strong> Imports buildings and trees
							from WFS services
						</li>
						<li>
							<strong>Rasterization:</strong> Converts vector data to raster masks
						</li>
						<li>
							<strong>Height Extraction:</strong> Extracts building and tree heights
						</li>
						<li>
							<strong>Aspect Calculation:</strong> Calculates wind direction aspects
							(N/E/S/W)
						</li>
						<li>
							<strong>Grid Generation:</strong> Uses wind direction to pick aspect
							inputs and oriented grid cell dimensions
						</li>
					</ol>
					<p>
					...
					<ul>
						...
						<li>Minimum building height (default 5 m) and tree height (default 3 m)</li>
						<li>
							Optional grid cell width/height overrides (otherwise chosen by wind
							direction)
						</li>
						<li>
							Raster resolution (default 1 m) and tree buffer radius (default 3 m)
						</li>
						<li>
							Lambda weights and wind profile constants (defaults match current model)
						</li>
					</ul>
					<p>
