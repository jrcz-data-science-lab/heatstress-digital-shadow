...
export function TreesTab() {
  const [treesReferencePath, setTreesReferencePath] = useState(
    "/data/wind/height.tif",
  );
  const [treesOutputPath, setTreesOutputPath] = useState(
  ...
  const [rasterizeOutput, setRasterizeOutput] = useState(
    "/data/wind/trees-mask.tif",
  );
  ...
  const [treesMaskPath, setTreesMaskPath] = useState(
    "/data/wind/trees-mask.tif",
  );
  const [treesHeightOutput, setTreesHeightOutput] = useState(
  ...
  const [aspectHeightPath, setAspectHeightPath] = useState(
    "/data/wind/trees-height.tif",
  );
  const [aspectMaskPath, setAspectMaskPath] = useState(
    "/data/wind/trees-mask.tif",
  );
  const [aspectOutputDir, setAspectOutputDir] = useState("/data/wind");
  const [aspectWindDirection, setAspectWindDirection] =
    useState<WindDirection>("west");
  ...
  return (
    <div className="wind-panel__tab-content">
      ...
      >
        ...
        {importRequest.error && (
          <MessageBox message={importRequest.error} type="error" />
        )}
        ...
        >
          <p>
            This tool imports tree/vegetation objects from the PDOK BGT WFS
            service and exports them as point features in a GeoPackage.
          </p>
          <p>
          ...
          <ul style={{ marginLeft: "1rem" }}>
            <li>
              Loads the entire vegetatieobject_punt layer from BGT (no bbox
              limit)
            </li>
            <li>
              Filters features by the reference layer extent during export
            </li>
            <li>
              Exports trees within the extent as point features in a GeoPackage
            </li>
          </ul>
          <p>
            The reference layer is typically the height map generated in the
            Height Map tab. This approach avoids the 1000 object API limit by
            loading the full dataset and filtering locally.
          </p>
        </CollapsibleHelpBox>
      </WindSection>
      ...
      >
        ...
        >
          <p>
            This tool converts tree points from a GeoPackage to a binary raster
            mask with buffering.
          </p>
          <p>
            <strong>How it works:</strong>
          ...
          <ul style={{ marginLeft: "1rem" }}>
            ...
            <li>
              Buffers each point into a circular polygon (default 3m radius)
            </li>
            <li>
              Uses height map reference to match extent, resolution, and CRS
            </li>
            <li>Rasterizes buffered trees with default 1m resolution</li>
            <li>Creates a binary mask where trees = 1, no trees = 0</li>
          </ul>
          <p>
            <strong>Important:</strong> The height map reference ensures the
            mask aligns perfectly with the height map for later height
            extraction operations. The default 3m buffer represents typical tree
            canopy radius.
          </p>
        </CollapsibleHelpBox>
      </WindSection>
      ...
      >
        ...
        {extractRequest.error && (
          <MessageBox message={extractRequest.error} type="error" />
        )}
        ...
        >
          <p>
            This tool extracts tree heights from the height map using the trees
            mask.
          </p>
          <p>
            <strong>How it works:</strong>
          ...
          <ul style={{ marginLeft: "1rem" }}>
            <li>
              Applies the formula: (corrected DSM-DTM) * (trees-mask == 1)
            </li>
            <li>Creates a raster where only tree pixels have height values</li>
            <li>All non-tree pixels are set to 0</li>
          </ul>
          <p>
            This output layer contains only the heights of trees, which can be
            used for wind flow calculations, vegetation analysis, and urban
            forestry planning.
          </p>
        </CollapsibleHelpBox>
      </WindSection>
      ...
      >
        ...
        {aspectRequest.error && (
          <MessageBox message={aspectRequest.error} type="error" />
        )}
        ...
        >
          <p>
            Calculates tree-facing aspect and extracts only the selected wind
            direction mask.
          </p>
          <ul style={{ marginLeft: "1rem" }}>
            <li>
              Runs GDAL Aspect on the trees height layer (0-360°, true north)
            </li>
            <li>
              Bins into N=1 (315-45°), E=2 (45-135°), S=3 (135-225°), W=4
              (225-315°)
            </li>
            <li>
