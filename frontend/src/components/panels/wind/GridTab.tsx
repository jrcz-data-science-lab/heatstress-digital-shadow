...
export function GridTab() {
	...
	const [treesAspectPath, setTreesAspectPath] = useState('/data/wind/trees-aspect-west.tif');
	...
	const handleCreateGrid = () => {
		const gridWidthValue = getValidatedValue(
			requirePositiveNumber(gridWidth, 'Grid cell width must be a positive number'),
			request.setError,
		);
		...
		const gridHeightValue = getValidatedValue(
			requirePositiveNumber(gridHeight, 'Grid cell height must be a positive number'),
			request.setError,
		);
			...
			>
				...
				>
					...
					<ul style={{ marginLeft: '1rem' }}>
						<li>
							Create rectangular grid (default 125 m x 250 m) from height map extent
						</li>
						<li>
						...
						<li>
							Normalise <code>buildings_height_mean</code>: values below the threshold
							are raised to the minimum (default 5 m)
						</li>
						<li>
							Normalise <code>trees_height_mean</code>: values below the threshold are
							raised to the minimum (default 3 m)
						</li>
						<li>
							Lambda weights and wind profile constants are configurable for tuning
						</li>
					</ul>
					<p>
